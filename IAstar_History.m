    ...
    for i = 1:num_points
        ...
        for j = 1:num_points
            ...
            if FLAG == false
                ...
                % 后续：直接调用 IA*-H 增量更新
                [path, Open, Closed, g, parent,costA,path_length] = Astar_HIS(HeightData, start, goal, Open, Closed, g, parent, m, u, thetaM, thetaB);
            end
            % 保存结果
            paths{i,j} = path;
            if isempty(path)
...
function [path, Open, Closed, g, parent,costA,path_length] = Astar_HIS(HeightData, start, goal, Open, Closed, g, parent, m, u, thetaM, thetaB)
    % 目标已在 CLOSED，直接结束
    ...
    [path, Open, Closed, g, parent,costA,path_length] = Astar_continue(HeightData, start, goal, Open, Closed, g, parent, m, u, thetaM, thetaB);
end 
...
function [path, Open, Closed, g, parent,costA,path_length] = Astar_continue(HeightData, start, goal, Open, Closed, g, parent, m, u, thetaM, thetaB)
    %继续 A*
    costA = 0;
    while ~isempty(Open)
        [~,idx] = min(Open(:,3));
